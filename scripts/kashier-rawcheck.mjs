...
const shape = (s) =>
  s ? `len ${s.length}  ${s.slice(0, 6)}…${s.slice(-4)}` : "(empty)";
console.log("── keys ──");
...
if (appSecret && appSecret.length < secretKey.length) {
  console.log(
    `  ⚠ APP IS TRUNCATING the secret: app has ${appSecret.length} chars, file has ${secretKey.length}.`,
  );
  console.log(
    "    → the value is cut at a $ by dotenv-expand. Fix = escape EVERY $ as \\$ in .env,",
  );
...
async function tryOne(baseUrl) {
  ...
  try {
    ...
    console.log(
      "   ",
      j ? JSON.stringify(j).slice(0, 300) : text.slice(0, 300),
    );
    return ok;
  } catch (e) {
...
if (testOk)
  console.log(
    "✓ Keys are valid for TEST. Set KASHIER_BASE_URL=https://test-api.kashier.io",
  );
else if (liveOk)
  console.log(
    "✓ Keys are valid for LIVE. Set KASHIER_BASE_URL=https://api.kashier.io  (you were pointing at test!)",
  );
else
  console.log(
    "✗ Still rejected with the full secret → the key VALUES are wrong/expired, or api-key & secret are swapped in the dashboard. Regenerate keys in Kashier and re-copy.",
  );
...
if (testOk || liveOk) {
  ...
  console.log(
    `KASHIER_SECRET_KEY_B64=${Buffer.from(secretKey, "utf8").toString("base64")}`,
  );
  if (/\$/.test(apiKey)) {
    console.log(
      `KASHIER_API_KEY_B64=${Buffer.from(apiKey, "utf8").toString("base64")}`,
    );
  } else {
