...
const { db, authAdmin } = vi.hoisted(() => ({
    db: {
        tenantRevenue: { groupBy: vi.fn(), updateMany: vi.fn() },
...
beforeEach(() => {
    ...
    db.tenantRevenue.groupBy.mockResolvedValue([{ currency: "EGP", _sum: { amount: 250 }, _count: { _all: 3 } }]);
    db.settlement.create.mockResolvedValue({ id: "settle-1" });
    db.tenantRevenue.updateMany.mockResolvedValue({ count: 3 });
});

describe("POST /api/revenue/settle", () => {
    it("401 when not an admin", async () => {
        authAdmin.mockResolvedValue(false);
        expect((await post({ tenantSlug: "acme" })).status).toBe(401);
        expect(db.settlement.create).not.toHaveBeenCalled();
    });

    it("400 on an invalid academy slug", async () => {
        expect((await post({ tenantSlug: "Bad!" })).status).toBe(400);
    });

    it("creates a Settlement per currency and links the outstanding LIVE rows", async () => {
        const res = await post({ tenantSlug: "acme", method: "instapay", reference: "ipn-9", note: "sept" });
        ...
        expect(sc).toMatchObject({ tenantSlug: "acme", currency: "EGP", amount: 250, txnCount: 3, method: "instapay", reference: "ipn-9", note: "sept" });

        // Rows flipped to settled and linked to the settlement; only unsettled LIVE rows.
        const upd = db.tenantRevenue.updateMany.mock.calls[0][0];
        expect(upd.where).toMatchObject({ tenantSlug: "acme", status: "paid", settled: false, mode: "live", currency: "EGP" });
    ...
    it("returns empty when nothing is outstanding (no settlement created)", async () => {
        db.tenantRevenue.groupBy.mockResolvedValue([]);
        const res = await post({ tenantSlug: "acme" });
    ...
    it("defaults method to bank and applies currency/orderIds/upToPaidAt filters", async () => {
        await post({ tenantSlug: "acme", currency: "usd", orderIds: ["o1", "o2"], upToPaidAt: "2026-09-01T00:00:00Z" });
        ...
        const where = db.tenantRevenue.groupBy.mock.calls[0][0].where;
    ...
    it("settles test revenue only when mode=test is explicit", async () => {
        await post({ tenantSlug: "acme", mode: "test" });
        expect(db.tenantRevenue.updateMany.mock.calls[0][0].where.mode).toBe("test");
    });
});
