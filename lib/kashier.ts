...
export async function createSession(input: CreateSessionInput): Promise<CreateSessionResult> {
    ...
    if (!merchantId || !apiKey || !secretKey) {
        return { ok: false, error: "Kashier is not configured (KASHIER_MERCHANT_ID / API_KEY / SECRET_KEY)" };
    }
    ...
    try {
        ...
        if (res.status < 200 || res.status >= 300) {
            return { ok: false, error: `Kashier session creation failed: HTTP ${res.status}`, raw };
        }
 ...
 */
export function verifyWebhook(payload: string, signatureHeader: string): WebhookVerdict {
    const empty: WebhookVerdict = {
        signatureValid: false, eventType: "", merchantOrderId: "", kashierOrderId: "",
        transactionId: "", status: "", amount: 0, currency: "", raw: null,
    };
