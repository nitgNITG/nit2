    ...
    try {
        await fetch(url, {
            ...
            body: JSON.stringify({
                ...
                homepageTemplate,
                // {text,href} for the template's editable hooks; create.sh runs apply_homepage_content.php.
                homepageContent,
...
export async function triggerExpiryReminder(
    ...
    opts?: {
        ...
        subscribedAt?: string; // YYYY-MM-DD → local_license/subscribedat; "" leaves it
    },
): Promise<void> {
    ...
    try {
        ...
        await fetch(url.toString(), {
            ...
            body: JSON.stringify({
                ...
                subscribed_at: opts?.subscribedAt ?? "",
            }),
        });
    } catch (e) {
...
export type ProvisionInput = {
    ...
    /** {text,href} content for the template's editable hooks (optional). */
...
export async function provisionAcademy(input: ProvisionInput): Promise<ProvisionResult> {
    ...
    }, input.platformLang, adminPassword, integrations, nitAdminPassword, input.homepageTemplate ?? "t1", input.homepageContent ?? null);
