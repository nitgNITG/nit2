...
// GET /api/revenue/transactions?tenantSlug=&currency=&settled=all|true|false&from=&to=&limit=
...
export async function GET(req: NextRequest) {
    ...
    const tenantSlug = (url.searchParams.get("tenantSlug") ?? "").trim().toLowerCase();
    if (!SLUG_RE.test(tenantSlug)) {
        return NextResponse.json({ error: "invalid tenantSlug" }, { status: 400 });
    }

    const where: any = { tenantSlug, status: "paid" };
    ...
    try {
        const [rows, academy] = await Promise.all([
            ...
            prisma.tenant.findUnique({ where: { slug: tenantSlug }, select: { name: true } }),
        ]);
        return NextResponse.json({ academyName: academy?.name ?? tenantSlug, transactions: rows });
    } catch (e) {
