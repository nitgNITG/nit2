                    ...
                    <div className='text-center space-y-6 max-w-3xl px-4'>
                        <div className='flex justify-center'>
                            <span className='inline-flex items-center gap-2 border border-[#1E7D67] text-[#00FFCD] text-xs font-semibold tracking-widest uppercase px-4 py-2 rounded-full bg-[#1E7D67]/10 backdrop-blur-sm'>
                                ...
                                {isAr ? 'منصات تعليمية' : 'Educational Platforms'}
                            </span>
                        </div>
                        <div>
                            <h1 className='text-xl md:text-2xl lg:text-3xl 2xl:text-5xl font-bold leading-tight drop-shadow-lg'>
                                <span className='text-white'>{isAr ? 'منصات تعليمية' : 'Educational'}</span>
                                <span className='block mt-2' style={{ color: '#00FFCD' }}>
                                    {isAr ? 'تصميم وتطوير منصات التعلم الإلكتروني' : 'Platform Development & Solutions'}
                                </span>
                            </h1>
                        </div>
                        <p className='text-white/80 text-base md:text-lg font-medium max-w-xl mx-auto leading-relaxed drop-shadow'>
                            {isAr
                                ? 'نطوّر منصات تعليمية مخصصة للجامعات والمدارس والمؤسسات — أنظمة LMS متكاملة مع بث مباشر واختبارات وشهادات إلكترونية'
                                : 'Custom educational platforms for universities, schools & institutions — integrated LMS with live streaming, assessments & e-certificates'}
                        </p>
                        <div className='flex flex-wrap gap-8 justify-center pt-2'>
                            <div className='text-center'>
                                <div className='text-lg md:text-xl 2xl:text-2xl font-bold drop-shadow' style={{ color: '#00FFCD' }}>30+</div>
                                <div className='text-white/70 text-sm'>{isAr ? 'منصة تعليمية' : 'Educational platforms'}</div>
                            </div>
