'use client'

import React, { useState } from 'react'
import { useLocale } from 'next-intl'
import toast from 'react-hot-toast'

export default function ForgotPasswordPage() {
    const locale = useLocale()
    const isAr = locale === 'ar'
    const tr = (ar: string, en: string) => (isAr ? ar : en)

    const [step, setStep] = useState<1 | 2>(1)
    const [email, setEmail] = useState('')
    const [code, setCode] = useState('')
    const [password, setPassword] = useState('')
    const [confirm, setConfirm] = useState('')
    const [busy, setBusy] = useState(false)

    const requestCode = async (e?: React.FormEvent) => {
        e?.preventDefault()
        if (!email.trim()) return
        setBusy(true)
        try {
            const res = await fetch('/api/auth/forgot-password', {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ email: email.trim().toLowerCase(), locale }),
            })
            const d = await res.json().catch(() => ({}))
            if (!res.ok) { toast.error(d?.message || tr('حصل خطأ.', 'Something went wrong.')); return }
            toast.success(d?.message || tr('لو البريد مسجّل، هيوصلك رمز.', 'If registered, a code was sent.'))
            setStep(2)
        } catch { toast.error(tr('تعذّر الاتصال.', 'Network error.')) } finally { setBusy(false) }
    }

    const resetPassword = async (e?: React.FormEvent) => {
        e?.preventDefault()
        if (!/^\d{6}$/.test(code.trim())) { toast.error(tr('الرمز 6 أرقام.', 'Code is 6 digits.')); return }
        if (password.length < 8) { toast.error(tr('كلمة السر 8 أحرف على الأقل.', 'Password must be 8+ characters.')); return }
        if (password !== confirm) { toast.error(tr('كلمتا السر غير متطابقتين.', 'Passwords do not match.')); return }
        setBusy(true)
        try {
            const res = await fetch('/api/auth/reset-password', {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ email: email.trim().toLowerCase(), code: code.trim(), password }),
            })
            const d = await res.json().catch(() => ({}))
            if (!res.ok) { toast.error(d?.message || tr('الرمز غير صحيح.', 'Invalid code.')); return }
            toast.success(tr('تم تغيير كلمة السر، سجّل الدخول.', 'Password changed — please sign in.'))
            window.location.href = `/${locale}/account`
        } catch { toast.error(tr('تعذّر الاتصال.', 'Network error.')) } finally { setBusy(false) }
    }

    return (
        <div dir={isAr ? 'rtl' : 'ltr'} className='min-h-[70vh] bg-[#0B2923] flex items-center justify-center px-4 py-16'>
            <div className='w-full max-w-md rounded-3xl bg-[#F5F3EE] p-8 shadow-2xl ring-1 ring-white/10'>
                <h1 className='text-2xl font-extrabold text-[#0B2923]'>{tr('نسيت كلمة السر', 'Forgot password')}</h1>
                <p className='mt-1 text-sm text-[#0B2923]/60'>
                    {step === 1
                        ? tr('اكتب بريدك ونبعتلك رمز إعادة التعيين.', 'Enter your email and we’ll send a reset code.')
                        : tr('اكتب الرمز اللي وصلك وكلمة السر الجديدة.', 'Enter the code we sent and your new password.')}
                </p>

                {step === 1 ? (
                    <form onSubmit={requestCode} className='mt-6 space-y-4'>
                        <Field label={tr('البريد الإلكتروني', 'Email')}>
                            <input type='email' dir='ltr' autoComplete='email' value={email} onChange={(e) => setEmail(e.target.value)}
                                className={input} placeholder='you@example.com' />
                        </Field>
                        <button type='submit' disabled={busy} className={btnPrimary}>
                            {busy ? tr('جارٍ الإرسال…', 'Sending…') : tr('إرسال الرمز', 'Send code')}
                        </button>
                    </form>
                ) : (
                    <form onSubmit={resetPassword} className='mt-6 space-y-4'>
                        <Field label={tr('الرمز', 'Code')}>
                            <input inputMode='numeric' dir='ltr' maxLength={6} value={code}
                                onChange={(e) => setCode(e.target.value.replace(/\D/g, ''))}
                                className={`${input} tracking-[0.4em] text-center font-bold`} placeholder='••••••' />
                        </Field>
                        <Field label={tr('كلمة السر الجديدة', 'New password')}>
                            <input type='password' autoComplete='new-password' value={password} onChange={(e) => setPassword(e.target.value)} className={input} placeholder='••••••••' />
                        </Field>
                        <Field label={tr('تأكيد كلمة السر', 'Confirm password')}>
                            <input type='password' autoComplete='new-password' value={confirm} onChange={(e) => setConfirm(e.target.value)} className={input} placeholder='••••••••' />
                        </Field>
                        <button type='submit' disabled={busy} className={btnPrimary}>
                            {busy ? tr('جارٍ…', '…') : tr('تغيير كلمة السر', 'Change password')}
                        </button>
                        <button type='button' onClick={requestCode} disabled={busy} className='w-full text-sm font-bold text-[#1E7D67] hover:underline'>
                            {tr('لم يصلك الرمز؟ أعد الإرسال', 'Didn’t get it? Resend code')}
                        </button>
                    </form>
                )}

                <p className='mt-6 text-center text-sm'>
                    <a href={`/${locale}/account`} className='font-bold text-[#1E7D67] hover:underline'>
                        {tr('العودة لتسجيل الدخول', 'Back to sign in')}
                    </a>
                </p>
            </div>
        </div>
    )
}

const input = 'w-full rounded-xl border border-black/10 bg-white px-4 py-3 text-[#0B2923] outline-none transition-colors focus:border-[#1E7D67] focus:ring-2 focus:ring-[#1E7D67]/20'
const btnPrimary = 'w-full rounded-xl bg-gradient-to-b from-[#1E7D67] to-[#0B2923] px-6 py-3.5 text-base font-extrabold text-[#00FFB2] transition-transform hover:scale-[1.01] disabled:opacity-60'

function Field({ label, children }: { label: string; children: React.ReactNode }) {
    return (
        <label className='block'>
            <span className='mb-1.5 block text-sm font-bold text-[#0B2923]'>{label}</span>
            {children}
        </label>
    )
}
